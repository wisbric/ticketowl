...
// It validates the HMAC-SHA1 signature, parses the payload, and pushes
// the event to the tenant's Redis stream synchronously.
func HandleZammad(rdb *redis.Client, webhookSecret string, logger *slog.Logger) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// 2. Validate HMAC-SHA1 signature before doing anything else.
		...
		// 4. Push to Redis stream synchronously.
